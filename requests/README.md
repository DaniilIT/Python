...
**URL** - Uniform Resource Locator - унифицированный указатель ресурсов, то есть ссылка на объект в Интернете.
**URI** - Uniform Resource Identitier - идентификатор ресурса = URL без query параметров.
**Endpoint** - URI с указанием метода, однозначно определяющий view на сервере, которая будет обрабатывать зарос.
...
* **POST** - отправка сущностей в теле запроса для создания ресурса;
* **PUT** - изменение ресурса (всех полей);
...
\- споcоб организации **API**.\
 **Application programming interface** - интерфейс прикладного уровня, набор протоколов, которые определяют взаимодействие между приложениями.\
**REST** - (это не спецификация и не стандарт) это стиль веб-архитектуры (парадигма), который описывает принципы взаимодействия клиента и сервера.\
**REpresentational State Transfer** - передача самоописываемого состояния.\
*RESTful Service* - сервис, у которого API организован при помощи REST.

**принципы REST**:
* Client - Server взаимодействие (основан на HTTP).
* каждый ресурс однозначно определяется глобальным идентификатором URL.
* нет состояний (stateless)
* возможность кешируемости (cacheable)

**правила REST**:
1) Используете множественное число, например `/users`
2) Не используете глаголы, никаких get, set, add и т.д.
3) Используете только строчные буквы (lowercase), в крайних случаях `_`
4) Старайтесь делать короткие URL (должен быть небольшой уровень вложенности)
...
\- свойство, когда клиенты делают неоднократно один и тот же запрос и получают один и тот же ответ.

* GET, HEAD, OPTIONS, TRACE - безопасные методы и идемпотентные
* PUT, DELETE - небезопасные методы и идемпотентные
* POST PATCH - небезопасные методы и неидемпотентные

Кроме REST API есть еще много других, например SOAP API, XML-RPC API, которые используют XML формат для передачи информации.

**SWAGGER** - описательный фреймворк для спецификации RESTful API.
