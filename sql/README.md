...
* AGE(current_date, col) – возвращает INTERVAL в формате TIMESTAMP
    current_date - текущяя дата, NOW() - дата и время
* COALEDCE(col1, '-') – возвращает первое не NULL значение;

* CAST(col AS DATE) – приведение типов (можно через ::);
* ROUND(col, 2) – округление до 2-х знаков после запятой.
    при делении необходимо привести одно число к DECIMAL;
